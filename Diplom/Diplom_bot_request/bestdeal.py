...
def get_hotel_info(message:telebot.types.Message, current_bot: telebot) -> None:
    ...
    get_range_price(message, hotels_dict, current_bot)

def get_range_price(message:telebot.types.Message, hotels_dict: Dict, current_bot: telebot) -> None:
    ...
    bot.register_next_step_handler(message, get_range_distance, current_bot)

def get_range_distance(message:telebot.types.Message, current_bot: telebot) -> None:
    ...
    try:
        if len(message.text.split('-')) == 2 and int(message.text.split('-')[0]) <= int(message.text.split('-')[1]):
            ...
            bot.register_next_step_handler(message, get_number_city, current_bot)
        else:
            raise
    except:
...
def get_number_city(message:telebot.types.Message, current_bot: telebot) -> None:
